...
from ml.scorer import extract_text_from_pdf, compute_final_score, extract_skills
...
def upload_resume():
    ...
    try:
        ...
        log(cand.id, 'UPLOAD_RESUME', filename)
...
def apply_to_job(jid):
    ...
    try:
        ...
        match_pct = scores['match_percentage']
        ...
        log(cand.id, 'APPLY_JOB', f'{job.title} Score:{match_pct}')
        ...
        print("[APPLY ERROR]", traceback.format_exc())
...
def my_applications():
    ...
    for a in apps:
        ...
        rank = next(
            (i + 1 for i, x in enumerate(all_apps) if x.id == a.id),
            None
        )

        d['rank'] = rank
        d['total_applicants'] = len(all_apps)
...
def recommend_jobs():
    ...
    recs = sorted(recs, key=lambda x: x["skill_match"], reverse=True)
